parameters HOUR=0.1 TSTOP=3600*HOUR VDIG=1 LAMBDA=0 FRAND=1 D=0 F0=60 H_COEFF=1 
parameters PRAND=1M
...
Sp control begin
    ...
    tau = 2.0;            %relaxation time     --> tau = 1/alpha
    c  = sqrt(0.005);     %diffusion constant  --> b   = c^2
...
%compute x and y
    ...
    k0 = exp(-dt/tau);
    k1 = sqrt((c*tau*0.5)*(1-(exp(-dt/tau))^2));

    for k=2:length(T)
       x(k) = k0*x(k-1) + k1*r1(k);
    end
...
#ifdef SYNT
    Dc dc print=yes nettype=yes load="Pf.init" fixsm=yes iabstol=1u
    Pz pz nettype=yes

    Tr tran stop=TSTOP nettype=yes method=2 maxord=2 noisefmax=FRAND/2 \
	    noiseinj=2 seed=12345 iabstol=1u devvars=1 tmax=0.1 
#else
    Dc dc print=yes nettype=yes fixsm=yes iabstol=1u
    Pz pz nettype=yes

    Tr tran stop=TSTOP nettype=yes method=2 maxord=2 noisefmax=FRAND/2 \
	    noiseinj=2 seed=12345 iabstol=1u devvars=1 tmax=0.1 savelist=SaveSet
#endif

    save( "mat5", MATF, "time", get("Tr.time:noise"), \
                        "omega01",  get("Tr.omega01:noise"),  \
			"omega02",  get("Tr.omega02:noise"),  \
			"omega03",  get("Tr.G3:omega:noise"),  \
			"omega06",  get("Tr.G6:omega:noise"),  \
			"omega08",  get("Tr.G8:omega:noise"),  \
			"omegacoi", get("Tr.omegacoi:noise") );
...
Dfr14   drift14  gnd  vsource vsin=-0*0.12 freq=1/(HOUR*3600)
...
Wav13    rand13  gnd   port noisesamples="Wave1"
